...
export default function StatCards({ expenses, transactions, balances, totalSpent, myBalance }) {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      <div className="bg-[#1A1F2E]/60 backdrop-blur-sm border border-slate-800/80 rounded-2xl p-4 lg:p-5 shadow-lg">
        ...
        <p className="text-2xl lg:text-3xl font-bold text-white tracking-tight">₹{Math.round(totalSpent || 0)}</p>
      </div>
      <div className="bg-[#1A1F2E]/60 backdrop-blur-sm border border-slate-800/80 rounded-2xl p-4 lg:p-5 shadow-lg">
        <p className="text-slate-400 text-sm font-medium mb-1.5">{myBalance >= 0 ? "you get" : "you owe"}</p>
        <p className={`text-2xl lg:text-3xl font-bold tracking-tight ${myBalance >= 0 ? 'text-teal-400' : 'text-red-400'}`}>
          ₹{Math.round(Math.abs(myBalance))}
        </p>
      </div>
      <div className="bg-[#1A1F2E]/60 backdrop-blur-sm border border-slate-800/80 rounded-2xl p-4 lg:p-5 shadow-lg">
